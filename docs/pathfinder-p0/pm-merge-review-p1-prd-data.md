...
主 Agent 建议进入 P1-A 产品化拆分，但需要先由产品负责人确认 5 个决策点。确认前不发布前端 / 后端开发任务。
...
2. P1-A 固定 `p0-xiaoc-opendocuments@1.0.0` TrialPackage，OpenDocuments 仍是只读公开参考项目。
...
1. 背景输入：真实用户可填写背景，小 C 可一键填充为示例。
2. 固定试航包：只读加载 `p0-xiaoc-opendocuments@1.0.0`。
3. 固定路径：三条路径固定展示，不做重新推荐。
4. 证据链：展示 JD 样例证据、用户背景证据、OpenDocuments 公开来源、风险边界和下一步试航。
5. 固定 6 问：用户完成自己的作答，可保留示例作答作为参考。
...
2. `FE-002`：真实用户输入与固定试航包产品化
   - 产物：背景输入表单、小 C 示例填充、固定 TrialPackage 展示、结果页真实用户文案、Markdown 使用用户快照。
...
4. `QA-002`：P1-A 产品化回归计划
   - 产物：真实用户输入、示例填充、保存恢复、缺项阻断、反包装阻断、Markdown 导出、移动端和 fallback 的验收用例。

## 8. 产品负责人待确认

在发布下一轮开发任务前，请产品负责人确认：

1. P1-A 是否按“允许替换用户背景，但不开放推荐”推进。
2. P1-A 第一轮是否暂不接 LLM，只保留 LLM 为第二轮可选草稿辅助。
3. P1-A 是否继续复用 `analysis_records`，P1-B 再写专用表 migration PRD。
4. P1-A 是否默认匿名化展示 JD 公司，只保留“样例 JD / 样本趋势参考”。
5. OpenDocuments License 核验是否由数据任务维护 fixture，并以核验日期作为展示依据。
