...
import { ArrowLeft, BriefcaseBusiness, Check, ChevronRight, CircleDot, FileText, FolderKanban, GitMerge, Loader2, Quote, Radar, RotateCcw, Scissors, Sparkles } from "lucide-react";
        ...
        <main>
          <section className="mb-10 border-t-4 border-studio-accent bg-studio-paper px-5 py-6 shadow-[4px_4px_0_#ddd5c6] sm:px-7">
            <div className="flex flex-col gap-5 sm:flex-row sm:items-start sm:justify-between">
              ...
              >
                {analyzingTarget ? <Loader2 className="h-4 w-4 animate-spin" /> : <Sparkles className="h-4 w-4" />}
                {analyzingTarget
                  ? "正在阅读 JD"
                  : snapshot.target_analysis.status === "completed"
                    ? "重新提取"
                    : snapshot.target_analysis.status === "failed"
                      ? "重新尝试"
                      : "提取岗位信号"}
              </button>
            </div>

            {snapshot.target_analysis.status === "failed" && snapshot.target_analysis.last_error && (
              <div className="mt-6 flex flex-col gap-3 border-l-4 border-studio-accent bg-[#fff1ec] px-4 py-3 text-sm text-[#7e2c1b] sm:flex-row sm:items-center sm:justify-between">
                <span>{snapshot.target_analysis.last_error.message}</span>
                <span className="shrink-0 text-xs font-semibold">现有材料未被改动</span>
              </div>
            )}

            {snapshot.role_signals.length ? (
              <div className="mt-7 grid gap-4 xl:grid-cols-3">
                {snapshot.role_signals.map((signal, index) => (
              ...
              <div className="mt-6 grid gap-3 border-y border-studio-line/10 py-4 text-sm text-studio-muted sm:grid-cols-[minmax(0,1fr)_auto] sm:items-center">
                <span>{snapshot.target_analysis.status === "completed" ? "这份 JD 没有支持足够明确的优先信号；系统不会为了凑数补写弱结论。" : "先保留原始 JD，再由 AI 判断最值得优先回应的岗位要求；信号不足时不会凑满三条。"}</span>
