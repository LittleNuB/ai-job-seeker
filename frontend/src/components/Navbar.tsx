...
export default function Navbar() {
  ...
  return (
    <nav className="sticky top-0 z-50 border-b border-[#17211b]/15 bg-[#fffdf8]/95 backdrop-blur">
      <div className="mx-auto max-w-7xl px-3 sm:px-4">
        <div className="flex h-14 items-center justify-between">
          <Link href="/" className="flex items-center gap-2 text-base font-bold text-[#17211b] sm:text-lg">
          ...
          <div className="hidden items-center gap-1 lg:flex">
              ...
              return (
                <Link
                  ...
                  className={`flex items-center gap-1.5 rounded-md px-3 py-2 text-sm font-medium transition-colors ${
                    active
                      ? "bg-[#e4efcf] text-[#2e472f]"
                      : "text-[#59675e] hover:bg-[#f0ece3] hover:text-[#17211b]"
                  }`}
                >
            ...
            {email ? (
              <button
                ...
                className="flex items-center gap-1.5 rounded-md px-3 py-2 text-sm font-medium text-[#59675e] transition-colors hover:bg-[#f0ece3] hover:text-[#17211b]"
                title={`当前账号：${email}`}
              >
          ...
          {email ? (
            <button
              type="button"
              onClick={handleLogout}
              className="flex items-center gap-1.5 rounded-md px-2.5 py-2 text-sm font-medium text-[#59675e] transition-colors hover:bg-[#f0ece3] hover:text-[#17211b] lg:hidden"
              title={`当前账号：${email}`}
            >
              <LogOut className="h-4 w-4" />
              退出
        ...
        <div className="flex items-center gap-1 overflow-x-auto border-t border-[#17211b]/10 py-1 lg:hidden">
          {links.map(({ href, label, icon: Icon }) => {
            const active = href === "/" ? pathname === "/" : pathname.startsWith(href);
            return (
              <Link
